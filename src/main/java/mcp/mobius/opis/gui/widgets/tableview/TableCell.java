...
class TableCell extends WidgetBase {
    ...
    public TableCell(IWidget parent, String text, WAlign align, float fontSize) {
        ...
        */
        ...
        switch (align) {
            case CENTER:
                label.setGeometry(new WidgetGeometry(
                        50.0, 50.0, 100.0, 100.0, CType.RELXY, CType.RELXY, WAlign.CENTER, WAlign.CENTER));
                label.setScale(this.fontSize);
                break;
            case LEFT:
                label.setGeometry(new WidgetGeometry(
                        5.0, 50.0, 95.0, 100.0, CType.RELXY, CType.RELXY, WAlign.LEFT, WAlign.CENTER));
                label.setScale(this.fontSize);
                break;
            case RIGHT:
                label.setGeometry(new WidgetGeometry(
                        5.0, 50.0, 95.0, 100.0, CType.RELXY, CType.RELXY, WAlign.RIGHT, WAlign.CENTER));
                label.setScale(this.fontSize);
                break;
            default:
                throw new UIException(String.format("Unexpected align value : %s", align));
